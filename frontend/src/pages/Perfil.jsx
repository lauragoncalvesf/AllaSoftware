...
export default function Perfil() {
  ...
  const carregarPerfil = async () => {
    try {
      ...
      setAviso({ titulo: "Erro", mensagem: "Não foi possível carregar o perfil." })
    } finally {
      setLoading(false)
    }
  }
...
const confirmarAlteracaoNome = async (e) => {
  ...
  const salvarSenha = async (e) => {
    ...
    if (!formSenha.senhaAtual || !formSenha.novaSenha || !formSenha.confirmarSenha) {
      setAviso({ titulo: "Atenção", mensagem: "Preencha todos os campos de senha." })
      return
    }

    if (formSenha.novaSenha !== formSenha.confirmarSenha) {
      setAviso({ titulo: "Atenção", mensagem: "A nova senha e a confirmação não coincidem." })
      return
    }

    if (formSenha.novaSenha.length < 6) {
      setAviso({ titulo: "Atenção", mensagem: "A nova senha deve ter pelo menos 6 caracteres." })
      return
    }
    ...
    try {
      ...
      setAviso({ titulo: "Sucesso", mensagem: "Senha alterada com sucesso!" })
    } catch (error) {
  ...
  return (
    <AppLayout>
      <div className="space-y-6 max-w-2xl">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-[#2D2E47]">Meu Perfil</h1>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <h2 className="text-lg font-semibold text-[#2D2E47] mb-4">Informações da conta</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            ...
            />
            <InfoItem label="Membro desde" valor={formatarData(perfil?.createdAt)} />
          </div>
        ...
        {/* Card de senha — só para usuários, não para login de empresa */}
        {perfil?.tipo === "usuario" && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
...
function InfoItem({ label, valor }) {
  return (
    <div>
      ...
      <p className="font-medium text-[#2D2E47] mt-1">{valor}</p>
    </div>
  )
}
